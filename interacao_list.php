...
/**
 *   	\file       interacao_list.php
 *		\ingroup    carteiracliente
 *		\brief      List page for interacao
 */
...
require_once __DIR__.'/class/interacao.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('carteiracliente', 'interacao', 'write');
	$permissiontodelete = $user->hasRight('carteiracliente', 'interacao', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("carteiracliente")) {
	accessforbidden('Module carteiracliente not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Interacao';
	$objectlabel = 'Interacao';
	$uploaddir = $conf->carteiracliente->dir_output;
...
$title = $langs->trans("Interacaos");
//$help_url = "EN:Module_Interacao|FR:Module_Interacao_FR|ES:Módulo_Interacao";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/carteiracliente/interacao_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-carteiracliente page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/carteiracliente/interacao_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendInteracaoRef";
$modelmail = "interacao";
$objecttmp = new Interacao($db);
